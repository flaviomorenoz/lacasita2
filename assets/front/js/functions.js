		...
		$.post(targeturl, { rowid: rowid, type: type, csrf_digi_name: token, ajax: '1' },
			function(data) {
				...
				if (data>0) {
					...
					message = "Artículo actualizado con éxito a su carrito";
					checkNotify('Carrito de Compra',message,'success');
						
				} else {	
					message = "El artículo no se actualizó";
					checkNotify('Carrito de Compra',message,'error');
				}
			}); 
	}
}
...
function addToCart(ref)
{
	...
	$.post(targeturl, { item_id: item_id, quantity: quantity, item_option_id: item_option_id, addon_ids: addon_ids, csrf_digi_name: token, ajax: '1',item_from:itemFrom },
		function(data) {
			...
			if (data==999) {
				message = "El artículo ya existía en el carrito";
 				checkNotify('Carrito de Compra',message,'info');
			    ...
			    message = "Artículo agregado exitosamente a su carrito";
			    checkNotify('Carrito de Compra',message,'success');
				
 			} else if (data!=999 && data<=0) {
				
				message = "Artículo no agregado a su carrito";
 				checkNotify('Carrito de Compra',message,'error');
			...
			function(data) {
				...
				$("body").removeClass("ajax-load");
	 			...
	 			if(data > 0) {
				    ...
				    message = "Cantidad de artículo actualizada a su carrito";
				    refresh_cart();
				    ...
				    checkNotify('Carrito de Compra',message,'success');
	 				...
	 				message = "Cantidad de artículo no actualizada";
			...
			$("body").removeClass("ajax-load");
 			...
 			if(data >= 0) {
			    ...
			    if (data > 0 && data!=999) {
					...
					$('#items_cnt').text(data);
					message = "Artículo eliminado exitosamente de su carrito";
					...
					message = "Artículo no eliminado de su carrito";
					checkNotify('Carrito de Compra',message,'error');
					location.reload();
					
				} else if(data==999) {
					
					message = "No hay artículos disponibles en su carrito";
					checkNotify('Carrito de Compra',message,'error');
					location.reload();
				}
